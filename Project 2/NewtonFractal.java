...
class NewtonFractal {
    ...
    public void printRoots() {
        ...
        int i;
    		String rooto = "";
        rooto = rooto + "(" +roots.get(0).toString()+ ")";
    		for (i=1; i<roots.size(); i++) {
    			rooto = rooto + ",(" +roots.get(i).toString()+ ")";
    		}
    }
    ...
    public Complex pixelToComplex(int i, int j) {
        ...
        return new Complex( i*dz + origin.getReal() , origin.getImag()- j*dz );
    }
      ...
      this.colorIterations = colorIterations;
  		for (int j = 0; j < NUMPIXELS; j++) {
  			for (int k = 0; k < NUMPIXELS; k++) {
  				Complex c = pixelToComplex(k, j);
  				iterator.iterate(c);
          Complex r = iterator.getRoot();
  				if (iterator.getError() == 0 && findRoot(r) == -1) {
  					roots.add(r);
  				} else if (iterator.getError() == 0 && findRoot(r) != -1){
  					colorPixel(k, j, findRoot(r), iterator.getNumIterations());
  				}
  			}
      	}
      }
    ...
    public static void main(String[] args) {
        ...
        Complex[] coeff = new Complex[] { new Complex(-1,0),new Complex(),new Complex(),new Complex(),
                                          new Complex(), new Complex(1,0)};
